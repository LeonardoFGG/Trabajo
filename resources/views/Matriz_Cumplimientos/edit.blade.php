                ...
                <form action="{{ route('matriz_cumplimientos.update', $cumplimiento->id) }}" method="POST">
                    ...
                    <div class="form-group mt-3">
                        ...
                        <select name="empleado_id" id="empleado_id" class="form-control" required>
                            ...
                            @foreach ($empleados as $empleado)
                                <option value="{{ $empleado->id }}" data-cargo-id="{{ optional($empleado->cargo)->id }}"
                        ...
                        <select name="cargo_id" id="cargo_id" class="form-control" required>
                            ...
                            @foreach ($cargos as $cargo)
                                <option value="{{ $cargo->id }}"
                                    {{ $cumplimiento->cargo_id == $cargo->id ? 'selected' : '' }}>
                                    {{ $cargo->nombre_cargo }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <select name="supervisor_id" id="supervisor_id" class="form-control" required>
                            ...
                            @foreach ($supervisores as $supervisor)
                                <option value="{{ $supervisor->id }}"
                                    {{ $cumplimiento->supervisor_id == $supervisor->id ? 'selected' : '' }}>
                        ...
                        <select name="parametro_id" id="parametro_id" class="form-control" required>
                            ...
                            @foreach ($parametros as $parametro)
                                <option value="{{ $parametro->id }}"
                                    {{ $cumplimiento->parametro_id == $parametro->id ? 'selected' : '' }}>
                                    {{ $parametro->nombre }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group mt-3">
                        ...
                        <input type="number" id="puntos" name="puntos" class="form-control"
                            value="{{ old('puntos', $cumplimiento->puntos) }}" min="0.5" step="0.5" required>
                    </div>
