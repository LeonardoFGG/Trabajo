...
class VentaController extends Controller
{
    ...
    public function create()
    {
        ...
        return response()->json([
            ...
            'productos' => $productos,
            'paquetes' => $paquetes,
            'html' => view('ventas.partials.create')->render()
        ]);
    }
    ...
    public function store(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), [
            ...
            'cliente_id' => 'required_if:tipo_venta,Interna|exists:clientes,id',
            'empleado_id' => 'required|exists:empleados,id',
            'nombre' => 'required_if:tipo_venta,Externa|string|max:255',
            'telefono' => 'required_if:tipo_venta,Externa|string|max:20',
            ...
            'paquetes.*' => 'integer|exists:paquetes,id',
            ...
            'archivo_propuesta' => 'nullable|file|mimes:pdf,doc,docx|max:2048',
            'archivo_negociacion' => 'nullable|file|mimes:pdf,doc,docx|max:2048',
            'anexo_contrato' => 'nullable|file|mimes:pdf,doc,docx|max:2048',
        ]);
        ...
        try {
            ...
            $ventaData = [
                ...
                'canal_comunicacion' => $request->canal_comunicacion,
                'fecha_presentacion' => $request->fecha_presentacion,
                'observacion_presentacion' => $request->observacion_presentacion,
            ...
            if ($request->hasFile('archivo_propuesta')) {
                $ventaData['archivo_propuesta'] = $this->guardarArchivo($request->file('archivo_propuesta'), 'propuestas');
            }

            if ($request->hasFile('archivo_negociacion')) {
                $ventaData['archivo_negociacion'] = $this->guardarArchivo($request->file('archivo_negociacion'), 'negociaciones');
            }

            if ($request->hasFile('anexo_contrato')) {
                $ventaData['anexo_contrato'] = $this->guardarArchivo($request->file('anexo_contrato'), 'contratos');
            }
            ...
            if ($request->has('productos')) {
                ...
                foreach ($request->productos as $productoId => $cantidad) {
                    if ($cantidad > 0) {
                        $productosConCantidad[$productoId] = [
                            'cantidad' => $cantidad,
                            'notas' => $request->notas_producto[$productoId] ?? null
                        ];
                    }
                }
                $venta->productos()->sync($productosConCantidad);
            }

            // Asociar paquetes con cantidades
            if ($request->has('paquetes')) {
                $paquetesConCantidad = [];
                foreach ($request->paquetes as $paqueteId => $cantidad) {
                    if ($cantidad > 0) {
                        $paquetesConCantidad[$paqueteId] = [
                            'cantidad' => $cantidad,
                            'notas' => $request->notas_paquete[$paqueteId] ?? null
                        ];
                    }
                }
                $venta->paquetes()->sync($paquetesConCantidad);
            }

            // Calcular y actualizar el monto total
            $venta->monto_total = $venta->calcularMontoTotal();
            $venta->save();
