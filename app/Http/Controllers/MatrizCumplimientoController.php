...
class MatrizCumplimientoController extends Controller
{
    public function index(Request $request)
    {
        ...
        // Cargar todos los empleados para el filtro si el usuario es admin o gerente general
        if ($user->isAdmin() || $user->isGerenteGeneral()) {
            $empleados = Empleados::orderBy('nombre1', 'asc')->get(); // Cargamos todos para el filtro
            $cumplimientosQuery = MatrizCumplimiento::with(['empleado', 'cargo', 'supervisor', 'parametro'])
                ->orderBy('created_at', 'desc');
        } elseif ($user->isEmpleado() && $user->isSupervisor()) {
            // Para supervisores, solo mostrar sus empleados supervisados
            $empleados = Empleados::where('supervisor_id', $user->empleado->id)
                ->orderBy('nombre1', 'asc')
                ->get();
            $cumplimientosQuery = MatrizCumplimiento::with(['empleado', 'cargo', 'supervisor', 'parametro'])
        ...
        $cumplimientos = $cumplimientosQuery->paginate(100);
    ...
    public function create(Request $request)
    {
        ...
        if ($user->isAdmin() || $user->isGerenteGeneral()) {
            $empleados = Empleados::with('cargo', 'supervisor')->orderBy('nombre1', 'asc')->get(); // Cargar relaciones
